...
CONST_LOWER_400 = REF_69
CONST_UPPER_400 = REF_72
CONST_LOWER_1000 = REF_69
CONST_UPPER_1000 = REF_72
CONST_FOR_VER = REF_84

CONST_OBJ_DIR_LOW = REF_72
CONST_OBJ_DIR_HIGH = REF_180
CONST_OBJ_FREQ_LOW = 100.0
CONST_OBJ_FREQ_HIGH = 500.0

CONST_SPL_MIN_GOAL = -25
NUM_PARTICLES = 1024
NUM_ITERATIONS = 500
...
def compute_additional_cost(spl_data, freq_data, num_angle, freq_min, freq_max, 
                           additional_cost, spl_threshold, condition='above'):
    """
    추가 비용을 계산하는 함수.

    Parameters:
    - spl_data (Tensor): [B, A, F] 형태의 SPL 데이터
    - freq_data (Tensor): [B, A, F] 형태의 주파수 데이터
    - num_angle (int): 각도 인덱스
    - freq_min (float): 주파수 최소값
    - freq_max (float): 주파수 최대값
    - additional_cost (float): 추가 비용 값
    - spl_threshold (float): SPL 임계값
    - condition (str): 'above' 또는 'below' 중 하나

    Returns:
    - Tensor: [B] 형태의 추가 비용
    """
    # 특정 num_angle의 spl과 freq 데이터 추출
    angle_spl = spl_data[:, num_angle, :]    # [B, F]
    angle_freq = freq_data[:, num_angle, :]  # [B, F]
    
    # 주파수 범위 마스킹
    freq_mask = (angle_freq >= freq_min) & (angle_freq <= freq_max)  # [B, F]
    
    # SPL 조건에 따른 마스킹
    if condition == 'above':
        spl_condition = angle_spl > spl_threshold
    ...
    # 전체 조건 마스킹
    combined_mask = spl_condition & freq_mask  # [B, F]
    
    # 배치별로 조건을 만족하는지 확인
    any_condition = combined_mask.any(dim=1).float()  # [B], 1.0 if 조건 만족
    
    # 추가 비용 계산
    additional_cost_tensor = any_condition * additional_cost  # [B]
    
    return additional_cost_tensor
...
def objective(batched_data_hor, batched_params, batched_data_ver):
    ...
    # ─────────────────────────────────────────────────────────────────
    # 3) horizontal의 additional_cost 계산 (vertical에는 없음)
    # ─────────────────────────────────────────────────────────────────
    # 추가 비용 조건을 그대로 사용
    # (아래 함수들에 들어가는 spl_data, freq_data는 [B, A, F] 형태여야 하므로 squeeze(1) 처리)
    spl_data_flat_h  = spl_hor.squeeze(1)   # [B, A, F]
    freq_data_flat_h = freq_hor.squeeze(1)  # [B, A, F]

    additional_cost_conditions = [
        {
            "num_angle": REF_90,
            "freq_min": 100.0,
            "freq_max": 200.0,
    ...
    # horizontal 추가비용
    additional_hor = torch.zeros_like(cost_hor)  # [B]
    for cond in additional_cost_conditions:
        add_c = compute_additional_cost(
            spl_data=spl_data_flat_h,
            freq_data=freq_data_flat_h,
            num_angle=cond["num_angle"],
            freq_min=cond["freq_min"],
            freq_max=cond["freq_max"],
            additional_cost=cond["additional_cost"],
            spl_threshold=cond["spl_threshold"],
            condition=cond["condition"]
        )
        additional_hor += add_c
    
    # ─────────────────────────────────────────────────────────────────
    # 4) vertical additional_cost 계산 
    # ─────────────────────────────────────────────────────────────────
    additional_ver = torch.zeros_like(cost_ver)  # [B]
    if VERTICAL_ADD_COST:
        spl_data_flat_v  = spl_ver.squeeze(1)   # [B, A, F]
        freq_data_flat_v = freq_ver.squeeze(1)  # [B, A, F]

        additional_cost_conditions_v = [
            {
                "num_angle": CONST_FOR_VER,
                "freq_min": 300.0,
                "freq_max": 450.0,
        ...
        for cond in additional_cost_conditions_v:
            add_c = compute_additional_cost(
                spl_data=spl_data_flat_v,
                freq_data=freq_data_flat_v,
                num_angle=cond["num_angle"],
                freq_min=cond["freq_min"],
                freq_max=cond["freq_max"],
                additional_cost=cond["additional_cost"],
            ...
            additional_ver += add_c

    # ─────────────────────────────────────────────────────────────────
    # 5) 최종 cost = (horizontal 평균 + 추가비용) + (vertical 평균)
    #    shape=[B], 맨 끝에 view(-1,1)로 만들어 [B,1]
    # ─────────────────────────────────────────────────────────────────
    cost = cost_hor + cost_ver + additional_hor + additional_ver
...
class ParticleSwarmOptimizer:
    def __init__(
        ...
        self.w_min = 0.4
        # 동적 관성 계수를 쓸지 여부
        self.use_adaptive_w = True
        
        # (2) Constriction Factor 사용 여부
        self.use_constriction = True

        # (3) 부분 랜덤 리셋을 위한 변수
        self.no_improvement_count = 0
        self.reset_threshold = 20  # 특정 횟수 이상 개선 없으면 부분 리셋

        # (4) 각 파라미터 인덱스별로 min_val, max_val을 정의
    ...
    def _create_bounds(self):
        ...
        for i in range(15):
            if i % 3 == 0:
                bounds_list.append([100.0, 1000.0])  # freq
            elif i % 3 == 1:
        ...
        bounds_list.append([200.0, 1000.0])  # index 76
    ...
    def _init_positions(self):
        """
        각 파라미터 범위 내에서 균일분포 or QMC(Sobol)로 초기화
        ...
        sobol_engine = qr.SobolEngine(dimension=num_params, scramble=True)
        qmc_samples = sobol_engine.draw(self.num_particles).to(self.device)
        positions = min_vals + (max_vals - min_vals) * qmc_samples

        # ---- 혹은 균일분포 random 사용(기존 방식) ----
    ...
    def _clamp_velocities(self, velocities: torch.Tensor) -> torch.Tensor:
        ...
        *동적 속도 클램핑 예시* (원한다면 적용)
          vel_scale = 0.2 * (1.0 - iteration / max_iter)
          ...
        """
        min_vals = self.bounds[:, 0].unsqueeze(0)
        max_vals = self.bounds[:, 1].unsqueeze(0)
    ...
    def optimize(self, objective_func, batched_data_ver):
        ...
        PSO 메인 루프 실행
        Args:
            objective_func (callable): 목적함수
                                       호출 시 shape=[num_particles, 1] 형태의 cost 반환
            batched_data_ver (Tensor): 검증용 데이터(입력과 동일 shape)
        """
        ...
        # (B) Constriction Factor 계산 (필요 시 사용)
        phi = self.c1 + self.c2
        # (phi^2 - 4phi) 가 음수가 되지 않도록 c1,c2를 적절히 세팅하거나, 안전 장치 필요
        # 예) if phi**2 < 4*phi: phi = 4.1
        if phi**2 < 4.0*phi:
            phi = 4.1
        ...
        while True:
            ...
            while True:
                ...
                self.no_improvement_count = 0  # 부분 리셋용 카운트
                ...
                for iteration in range(reset_interval):
                    ...
                    if self.use_adaptive_w:
                        # 선형 감소 예시
                        w_t = self.w_max - (self.w_max - self.w_min) * (current_iter / self.max_iter)
                    else:
                        w_t = self.w  # 고정 관성 계수
                    ...
                    # (iii) Constriction Factor 적용
                    if self.use_constriction:
                        new_velocities = chi * new_velocities

                    self.velocities = self._clamp_velocities(new_velocities)  # 속도 clamp
                    ...
                    # ---- (v) 부분 랜덤 리셋 ----
                    if self.no_improvement_count > self.reset_threshold:
                        # 임의로 몇 개 파티클 재초기화
                        idx_to_reset = torch.randint(0, self.num_particles, (5,))
                        self.positions[idx_to_reset] = self._init_positions()[idx_to_reset]
                        self.velocities[idx_to_reset] = self._init_velocities()[idx_to_reset]
                        self.pbest[idx_to_reset] = self.positions[idx_to_reset].clone()
                        self.pbest_cost[idx_to_reset] = float('inf')
                        self.no_improvement_count = 0
                        print(f"Partial Reinit triggered at iter={current_iter}")
            ...
            for iteration in range(remaining_iters):
                ...
                self.velocities = self._clamp_velocities(new_velocities)
